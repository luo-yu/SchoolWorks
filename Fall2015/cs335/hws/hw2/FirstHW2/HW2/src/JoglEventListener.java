...
public class JoglEventListener implements GLEventListener, KeyListener, MouseListener, MouseMotionListener {
	...
	public void display(GLAutoDrawable gLDrawable) {
		...
		if (scale_flag) {
			...
			for (int i = 0; i < 44; i++) {
				...
				gl.glVertex3f(p_center_x + scale_factor * (first1 - p_center_x) + ax,
						p_center_y + scale_factor * (first2 - p_center_y) + ay, first3);

				float second1 = vertices[(indices[i * 4 + 2]) * 3];
				float second2 = vertices[(indices[i * 4 + 2]) * 3 + 1];
				float second3 = vertices[(indices[i * 4 + 2]) * 3 + 2];

				gl.glVertex3f(p_center_x + scale_factor * (second1 - p_center_x) + ax,
						p_center_y + scale_factor * (second2 - p_center_y) + ay, second3);

				float third1 = vertices[(indices[i * 4 + 3]) * 3];
				float third2 = vertices[(indices[i * 4 + 3]) * 3 + 1];
				float third3 = vertices[(indices[i * 4 + 3]) * 3 + 2];
				gl.glVertex3f(p_center_x + scale_factor * (third1 - p_center_x) +ax,
						p_center_y + scale_factor * (third2 - p_center_y) +ay, third3);
			...
			for (int i = 0; i < 44; i++) {
				...
				gl.glVertex3f(p_center_x + (first1 - p_center_x) * cosTheta - (first2 - p_center_y) * sinTheta,
						p_center_x + (first1 - p_center_x) * sinTheta + (first2 - p_center_y) * cosTheta, first3);

				float second1 = vertices[(indices[i * 4 + 2]) * 3];
				float second2 = vertices[(indices[i * 4 + 2]) * 3 + 1];
				float second3 = vertices[(indices[i * 4 + 2]) * 3 + 2];

				gl.glVertex3f(p_center_x + (second1 - p_center_x) * cosTheta - (second2 - p_center_y) * sinTheta,
						p_center_x + (second1- p_center_x) * sinTheta + (second2 - p_center_y) * cosTheta, second3);

				float third1 = vertices[(indices[i * 4 + 3]) * 3];
				float third2 = vertices[(indices[i * 4 + 3]) * 3 + 1];
				float third3 = vertices[(indices[i * 4 + 3]) * 3 + 2];
				gl.glVertex3f(p_center_x + (third1 - p_center_x) * cosTheta - (third2 - p_center_y) * sinTheta,
						p_center_x + (third1 - p_center_x) * sinTheta + (third2 - p_center_y) * cosTheta, third3);
	...
	public void mouseDragged(MouseEvent e) {
		...
		ax = YY;
		ay = YY;
		double ax = XX - p_center_x;
		double ay = YY - p_center_y;
		double a = Math.pow(ax, 2) + Math.pow(ay, 2);

		scale_factor = (float) Math.sqrt(a);

		System.out.println("Current XX = " + XX + " Cureent YY = " + YY);
		System.out.println("p_center_x = " + p_center_x + " p_center_y = " + p_center_y);
	...
	public void mouseMoved(MouseEvent e) {
		...
		cosTheta = (float) (XX / Math.sqrt(Math.pow(XX, 2) + Math.pow(YY, 2)));
		cosTheta = (float) (YY / Math.sqrt(Math.pow(XX, 2) + Math.pow(YY, 2)));
		System.out.println("cosTheta " + cosTheta + " sinTheta " + sinTheta);

		System.out.println("Moved to : " + XX + ": " + YY);
		if (click_flag) {
			rotation_flag = true;
		}
	}

...
public class JoglEventListener implements GLEventListener, KeyListener, MouseListener, MouseMotionListener {
	...
	float currentEarthOrbitAngle = 0f;
	float currentMoonOrbitAngle = 0f;

	// Orbit radius
	float earthOrbitRadius = 5.0f;
	float moonOrbitRadius = 2.3f;
	...
	// Camera Mouse Difference
		...
		float cameraDeltaXOld = 0;
		float cameraDeltaYOld = 0;
		float camZoom = 4;
		float camZoomOld = 4;
		
		float earthAnimSpeed = 1f;
		float moonAnimSpeed = 2f;
	...
	public void display(GLAutoDrawable gLDrawable) {
		...
		if(isAnimate)
		{
			currentEarthOrbitAngle += earthAnimSpeed;
			currentMoonOrbitAngle -= moonAnimSpeed;
		}
		...
		// Camera Rotate
		gl.glPushMatrix();
		gl.glRotatef( cameraDeltaX, 0, 1, 0 );
		gl.glRotatef( cameraDeltaY, 1, 0, 0);
		...
		gl.glTranslatef(getXCoord(currentEarthOrbitAngle, earthOrbitRadius), 0,
				getZCoord(currentEarthOrbitAngle, earthOrbitRadius));
		gl.glScaled(0.33333, 0.33333, 0.33333);
		...
		gl.glTranslatef(getXCoord(currentMoonOrbitAngle, moonOrbitRadius), 0,
				getZCoord(currentMoonOrbitAngle, moonOrbitRadius));
		gl.glScaled(0.33333, 0.33333, 0.33333);
	...
	public void mouseDragged(MouseEvent e) {
		...
		if( mButton == -1) // Right Button for zoom
		{
			cameraDeltaX = ( XX - mouseStartX ) * 4 + cameraDeltaXOld;
			cameraDeltaY = ( mouseStartY - YY ) * 4 + cameraDeltaYOld;

...
public class JoglEventListener implements GLEventListener, KeyListener {
	...
	private boolean isTPressed = true;
	...
	public void display(GLAutoDrawable gLDrawable) {
		...
		// drawing circle using points
		// gl.glBegin(gl.GL_POINTS);
		// for(int i =0; i<1000; ++i){
		// gl.glVertex3f((float)(Math.cos(2*3.141592*i/1000.0)),
		// (float)(Math.sin(2*3.14159*i/1000.0)), 0);
		// }
		// gl.glEnd();
		...
		if (!isTPressed) {
			...
			for (i = 0; i <= triangleAmount; i++) {
				gl.glVertex2f(x, y);
				gl.glVertex2f(x + (radius * (float) Math.cos(i * twicePi / triangleAmount)),
						y + (radius * (float) Math.sin(i * twicePi / triangleAmount)));
			}
			gl.glEnd();
		}

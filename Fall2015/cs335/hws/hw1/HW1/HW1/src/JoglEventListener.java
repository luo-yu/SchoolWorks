...
public class JoglEventListener implements GLEventListener, KeyListener {
	...
	private double radius = 5, cx = 0.0, cy = 0.0;
	private int totalPts = 500;
	...
	public void display(GLAutoDrawable gLDrawable) {
		...
		gl.glClearColor(backrgb[0], 1, 0, 0);
		...
		// gl.glColor3f(1, 1, 1);
		// gl.glVertex3d(0, 0, 0);
		// gl.glVertex3f(vertices[indices[1]], vertices[indices[1+1]],
		...
		gl.glBegin(gl.GL_TRIANGLES);
		gl.glColor3f(0, 0, 1);
		for (int i = 1; i < indices.length + 1; i += 4) {
			gl.glVertex3f(vertices[(indices[i]) * 3], vertices[(indices[i]) * 3 + 1], vertices[(indices[i]) * 3 + 2]);
			gl.glVertex3f(vertices[(indices[i + 1]) * 3], vertices[(indices[i + 1]) * 3 + 1],
					vertices[(indices[i + 1]) * 3 + 2]);
			gl.glVertex3f(vertices[(indices[i + 2]) * 3], vertices[(indices[i + 2]) * 3 + 1],
		...
		for (int i = 0; i < totalPts; i++) {
			double angle = i * 1.0 / totalPts * 2 * Math.PI;

			gl.glVertex2d(Math.sin(angle) * radius + cx, Math.cos(angle) * radius + cy);
		}
